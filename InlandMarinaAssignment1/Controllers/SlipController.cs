...
namespace InlandMarinaAssignment1.Controllers
{
    public class SlipController : Controller
    {
        ...
        // USING JS / AJAX 
        public ActionResult FilterSlip(int id = 5)
        {
            try
            {
                ViewBag.Page = "Slips";
            ...
            {
                TempData["Message"] = "Error retrieving docks from Database";
                TempData["IsError"] = true;

            }
            return View(); // SLIPS ARE PASSED USING JS / AJAX
        }
        // To load our View Component USING JS / AJAX
        public ActionResult GetSlipsByDock(string dockId)
        {   
            int dockIdInt = Convert.ToInt32(dockId);
            // passes component name minues view component plus the integer for dock id
            return ViewComponent("SlipsByDock",dockIdInt);
        }
